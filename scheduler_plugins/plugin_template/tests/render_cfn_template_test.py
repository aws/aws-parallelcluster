...
def test_render_cfn_template():
    config = _read_yaml_file(os.path.join(os.path.dirname(__file__), "../examples/cluster_configuration.yaml"))
    logging.info("Configuration file:\n%s", _dump_yaml(config))
    rendered_template = _render_template(os.path.join(os.path.dirname(__file__), "../additional_cluster_infrastructure.cfn.yaml"), config)
    logging.info("Template:\n%s", dump_yaml(load_yaml(rendered_template)))
...
def _render_template(template, config):
    ...
    try:
        ...
        return (
            environment.get_template(os.path.basename(template)).render(cluster_configuration=config, cluster_name="name")
        )
