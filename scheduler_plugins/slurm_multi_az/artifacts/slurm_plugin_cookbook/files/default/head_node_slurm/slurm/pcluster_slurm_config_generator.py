...
def generate_instance_type_mapping_file(output_dir, queues):
    ...
    for queue in queues:
        queue_custom_settings = queue.get("CustomSettings") or {}
        instance_name_type_mapping[queue["Name"]] = {
            "SubnetIds": queue_custom_settings.get("SubnetIds", queue["Networking"]["SubnetIds"]),
            "CapacityType": queue.get("CapacityType", "ONDEMAND"),
            "AllocationStrategy": queue_custom_settings.get("AllocationStrategy", "lowest-price"),
            "ComputeResources": {},
        }
        compute_resources = queue["ComputeResources"]
        for compute_resource in compute_resources:
            instance_type = compute_resource.get("InstanceType")
            compute_custom_settings = compute_resource.get("CustomSettings") or {}
            instance_name_type_mapping[queue["Name"]]["ComputeResources"][compute_resource["Name"]] = {
                "InstanceType": instance_type,
                "InstanceTypes": compute_custom_settings.get("InstanceTypes", [instance_type]),
                "InstanceRequirements": compute_custom_settings.get("InstanceRequirements", {}),
            }
