...
def _addarg_nowait(subparser):
    subparser.add_argument("-nw", "--nowait", action='store_true',
...
def _get_parser():
    ...
    """
    parser = argparse.ArgumentParser(description='pcluster is the AWS ParallelCluster CLI and permits '
...
Examples::
  ...
  $ pcluster create mycluster --tags \'{ "Key1" : "Value1" , "Key2" : "Value2" }\'''')
    pcreate = subparsers.add_parser('create', help='Creates a new cluster.',
                                    formatter_class=argparse.RawDescriptionHelpFormatter, epilog=create_example)
    pcreate.add_argument("cluster_name", help='name for the cluster. The CloudFormation Stack name will be '
                                              'parallelcluster-[cluster_name]')
    _addarg_config(pcreate)
    _addarg_region(pcreate)
    _addarg_nowait(pcreate)
    pcreate.add_argument("-nr", "--norollback", action='store_true', default=False,
                         help='disable stack rollback on error')
    pcreate.add_argument("-u", "--template-url", help='specify URL for the custom CloudFormation template, '
                                                      'if it has been used at creation time')
    ...
    pupdate = subparsers.add_parser('update', help='Updates a running cluster by using the values in the config '
                                                   'file or a TEMPLATE_URL provided.',
    ...
    pupdate.add_argument("-nr", "--norollback", action='store_true', default=False,
    ...
    pupdate.add_argument("-rd", "--reset-desired", action='store_true', default=False,
    ...
    pdelete = subparsers.add_parser('delete', help='Deletes a cluster.',
                                    epilog='When the command is called and it starts polling for status of that call '
                                           'it is safe to "Ctrl-C" out. You can always return to that status by '
                                           'calling "pcluster status mycluster"')
    ...
    pstop = subparsers.add_parser('stop', help='Stops the compute fleet, leaving the master server running.',
    ...
    plist = subparsers.add_parser('list', help='Displays a list of stacks associated with AWS ParallelCluster.',
    ...
    ssh_example = textwrap.dedent('''Example::
...
Variables substituted::
  ...
  {ARGS} (only if specified on the cli)''')
    pssh = subparsers.add_parser('ssh', help='Connect to the master server using SSH.',
                                 ...
                                 formatter_class=argparse.RawDescriptionHelpFormatter, epilog=ssh_example)
    ...
    pami.add_argument("-ai", "--ami-id", dest="base_ami_id", required=True,
    ...
    pami.add_argument("-ap", "--ami-name-prefix", dest="custom_ami_name_prefix", default='custom-ami-',
                      help="specify the prefix name of the resulting AWS ParallelCluster AMI")
    pami.add_argument("-cc", "--custom-cookbook", dest="custom_ami_cookbook",
