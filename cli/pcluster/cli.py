...
from . import easyconfig, pcluster
...
def _addarg_region(subparser):
    subparser.add_argument(
        "--region", "-r", dest="region", help="specify a specific region to connect to", default=None
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="pcluster is a tool to launch and manage a cluster.",
    ...
    pcreate.add_argument(
        "cluster_name", type=str, default=None, help="create an AWS ParallelCluster with the provided name."
    )
    ...
    pcreate.add_argument(
        "--tags", "-g", type=json.loads, dest="tags", default=None, help="tags to be added to the stack"
    )
    pcreate.set_defaults(func=create)

    pupdate = subparsers.add_parser("update", help="update a running cluster")
    pupdate.add_argument(
        "cluster_name", type=str, default=None, help="update the AWS ParallelCluster with the provided name."
    )
    ...
    pdelete.add_argument(
        "cluster_name", type=str, default=None, help="delete the AWS ParallelCluster with the provided name."
    )
    ...
    pstart.add_argument(
        "cluster_name", type=str, default=None, help="starts the compute fleet of the provided cluster name."
    )
    ...
    pstop = subparsers.add_parser(
        "stop", help="stop the compute fleet, but leave the master server running for " "debugging/development"
    )
    pstop.add_argument(
        "cluster_name", type=str, default=None, help="stops the compute fleet of the provided cluster name."
    )
    ...
    pssh.add_argument("cluster_name", type=str, default=None, help="name of the cluster to set variables for.")
    pssh.add_argument(
        "--dryrun", "-d", action="store_true", dest="dryrun", default=False, help="print command and exit."
    )
