...
/usr/sbin/sshd -f /etc/ssh/sshd_config -h ${HOME}/.ssh/ssh_host_rsa_key
...
do
  if [[ ${ebs_shared_dir} != "NONE" ]]; then
    echo "Mounting shared file system ${ebs_shared_dir}"
...
echo "Starting the job as ${USER}..."
cd ${HOME}
echo "$@" | su ${USER} -c "/bin/bash"
