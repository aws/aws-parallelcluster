...
class S3Bucket:
    ...
    def check_bucket_is_bootstrapped(self):
        ...
        try:
            ...
            if e.error_code == "404":
                LOGGER.info(f"Bootstrap file doesn't exist. S3 Bucket {self.name} is not configured.")
                return False
            else:
...
class S3BucketFactory:
    ...
    def _check_default_bucket(cls, service_name: str, artifact_directory: str, stack_name: str):
        ...
        if not is_bootstrapped:
            ...
            LOGGER.info(f"Bucket {bucket.name} is already bootstrapped with required features.")
