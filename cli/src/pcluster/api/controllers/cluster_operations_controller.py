...
def describe_cluster(cluster_name, region=None):
    ...
    try:
        _get_login_nodes(cluster)
        login_nodes = _get_login_nodes(cluster)
        if login_nodes:
            response.login_nodes = login_nodes

    except ClusterActionError as e:
        # This should not be treated as a failure cause login nodes might not be running in some cases
        LOGGER.info(e)
...
def _get_login_nodes(cluster):
    ...
    if login_nodes_status and login_nodes_status.get_status():
        ...
        if login_nodes_status.get_address():
            login_nodes.address = login_nodes_status.get_address()
        if login_nodes_status.get_scheme():
            login_nodes.scheme = login_nodes_status.get_scheme()
        if login_nodes_status.get_healthy_nodes() or login_nodes_status.get_unhealthy_nodes():
