...
@pytest.mark.parametrize(
    ...
    "expected_efs_egress_rules, expected_fsx_egress_rules",
    [
        # Case 1: All nodes use the default (managed) security group
        # EFS ingress: rule(2049) * (head, compute, login1, login2) + rule(all) * (storage) = 5 rules
        # FSx ingress: rule(988,1018-1023) * (head, compute, login1, login2) + rule(all) * (storage) = 9 rules
        # EFS/FSx egress: rule(all) * (head, compute, login1, login2, storage) = 5 rules each
        (None, None, None, None, None, 5, 9, 5, 5),
        # Case 2: All nodes use the same custom security group (deduplication)
        # EFS ingress: rule(2049) * (customSG) + rule(all) * (storage) = 2 rules
        # FSx ingress: rule(988,1018-1023) * (customSG) + rule(all) * (storage) = 3 rules
        # EFS/FSx egress: rule(all) * (customSG, storage) = 2 rules each
        (
        ...
        # EFS ingress: rule(2049) * (head_sg, q1_sg, q2_sg, login1_sg, login2_sg) + rule(all) * (storage) = 6 rules
        # FSx ingress: rule(988,1018-1023) * (head_sg, q1_sg, q2_sg, login1_sg, login2_sg)
        # + rule(all) * (storage) = 11 rules
        # EFS/FSx egress: rule(all) * (head_sg, q1_sg, q2_sg, login1_sg, login2_sg, storage) = 6 rules each
        (
            "sg-1234567891234567a",
...
def test_storage_security_group_rules(  # noqa: C901
    mocker,
    test_datadir,
    pcluster_config_reader,
    head_sg,
    ...
    expected_efs_egress_rules,
    expected_fsx_egress_rules,
):
    """
    ...
    5. Egress rules allow all traffic to storage SG itself and to head/compute/login node SGs
    """
    mock_aws_api(mocker)
    mock_bucket(mocker)
    mock_bucket_object_utils(mocker)
    ...
    expected_source = set()
    expected_source.add(head_sg) if head_sg else expected_source.add(("Ref", "HeadNodeSecurityGroup"))
    expected_source.add(q1_sg) if q1_sg else expected_source.add(("Ref", "ComputeSecurityGroup"))
    expected_source.add(q2_sg) if q2_sg else expected_source.add(("Ref", "ComputeSecurityGroup"))
    expected_source.add(login1_sg) if login1_sg else expected_source.add(("Ref", "login1LoginNodesSecurityGroup"))
    expected_source.add(login2_sg) if login2_sg else expected_source.add(("Ref", "login2LoginNodesSecurityGroup"))
    ...
    assert_that(efs_source_sgs).is_equal_to(expected_source)
    ...
    # Verify source SGs match expected
    assert_that(fsx_source_sgs).is_equal_to(expected_source)

    # Build expected destination security groups for egress rules
    # Egress rules should allow traffic to storage SG itself and to head/compute/login node SGs
    expected_destinations = set()
    expected_destinations.add(head_sg) if head_sg else expected_destinations.add(("Ref", "HeadNodeSecurityGroup"))
    expected_destinations.add(q1_sg) if q1_sg else expected_destinations.add(("Ref", "ComputeSecurityGroup"))
    expected_destinations.add(q2_sg) if q2_sg else expected_destinations.add(("Ref", "ComputeSecurityGroup"))
    (
        expected_destinations.add(login1_sg)
        if login1_sg
        else expected_destinations.add(("Ref", "login1LoginNodesSecurityGroup"))
    )
    (
        expected_destinations.add(login2_sg)
        if login2_sg
        else expected_destinations.add(("Ref", "login2LoginNodesSecurityGroup"))
    )
    # Storage SG references itself - use a placeholder that will be matched per storage type
    expected_destinations.add(("Ref", "STORAGE_SG_SELF"))

    def _normalize_destination_sg(dest_sg):
        """Convert destination SG to a comparable format."""
        if isinstance(dest_sg, str):
            return dest_sg
        elif isinstance(dest_sg, dict) and "Ref" in dest_sg:
            return ("Ref", dest_sg["Ref"])
        return dest_sg

    # Test EFS egress rules - should allow all traffic to storage SG and head/compute/login SGs
    efs_egress_rules = get_resources(
        generated_template,
        type="AWS::EC2::SecurityGroupEgress",
        name_regex=r"^EFS.*SecurityGroup",
    )

    # Verify egress rule count
    assert_that(len(efs_egress_rules)).is_equal_to(expected_efs_egress_rules)

    efs_egress_destinations = set()
    for _name, rule in efs_egress_rules.items():
        props = rule["Properties"]
        # All egress rules should allow all traffic
        assert_that(props["IpProtocol"]).is_equal_to("-1")
        assert_that(props["FromPort"]).is_equal_to(0)
        assert_that(props["ToPort"]).is_equal_to(65535)
        # Collect destination security group
        dest_sg = _normalize_destination_sg(props.get("DestinationSecurityGroupId"))
        # Normalize storage SG self-reference
        if isinstance(dest_sg, tuple) and dest_sg[1].startswith("EFS") and dest_sg[1].endswith("SecurityGroup"):
            dest_sg = ("Ref", "STORAGE_SG_SELF")
        efs_egress_destinations.add(dest_sg)

    # Verify destination SGs match expected
    assert_that(efs_egress_destinations).is_equal_to(expected_destinations)

    # Test FSx egress rules - should allow all traffic to storage SG and head/compute/login SGs
    fsx_egress_rules = get_resources(
        generated_template,
        type="AWS::EC2::SecurityGroupEgress",
        name_regex=r"^FSX.*SecurityGroup",
    )
    ...
    # Verify egress rule count
    assert_that(len(fsx_egress_rules)).is_equal_to(expected_fsx_egress_rules)

    fsx_egress_destinations = set()
    for _name, rule in fsx_egress_rules.items():
        props = rule["Properties"]
        # All egress rules should allow all traffic
        assert_that(props["IpProtocol"]).is_equal_to("-1")
        assert_that(props["FromPort"]).is_equal_to(0)
        ...
        dest_sg = _normalize_destination_sg(props.get("DestinationSecurityGroupId"))
        # Normalize storage SG self-reference
        if isinstance(dest_sg, tuple) and dest_sg[1].startswith("FSX") and dest_sg[1].endswith("SecurityGroup"):
            dest_sg = ("Ref", "STORAGE_SG_SELF")
        fsx_egress_destinations.add(dest_sg)

    # Verify destination SGs match expected
    assert_that(fsx_egress_destinations).is_equal_to(expected_destinations)
