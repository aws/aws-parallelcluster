...
t = Template()
AvailabilityZone = t.add_parameter(
    Parameter(
        "AvailabilityZone",
        Type="String",
...
VolumeSize = t.add_parameter(
    Parameter("VolumeSize", Type="CommaDelimitedList", Description="Size of EBS volume in GB, if creating a new one")
)
...
EBSVolumeId = t.add_parameter(Parameter("EBSVolumeId", Type="CommaDelimitedList", Description="Existing EBS volume Id"))
EBSSnapshotId = t.add_parameter(
    Parameter(
        "EBSSnapshotId",
        Type="CommaDelimitedList",
...
EBSVolumeNum = t.add_parameter(
    Parameter(
        "NumberOfEBSVol", Type="Number", Description="Number of EBS Volumes the user requested, up to %s" % numberOfVol
    )
)
...
for i in range(numberOfVol):
    if i == 0:
        CreateVol = t.add_condition("Vol%s_CreateEBSVolume" % (i + 1), Equals(Select(str(i), Ref(EBSVolumeId)), "NONE"))
    elif i == 1:
        ...
        UseVol[i] = t.add_condition(
            "UseVol%s" % (i + 1), And(Not(Equals(Ref(EBSVolumeNum), str(i))), Condition(UseVol[i - 1]))
        )
        CreateVol = t.add_condition(
            "Vol%s_CreateEBSVolume" % (i + 1),
            And(Condition(UseVol[i]), Equals(Select(str(i), Ref(EBSVolumeId)), "NONE")),
    ...
    UseEBSPIOPS = t.add_condition("Vol%s_UseEBSPIOPS" % (i + 1), Equals(Select(str(i), Ref(VolumeType)), "io1"))
    UseVolumeSize = t.add_condition(
        "Vol%s_UseVolumeSize" % (i + 1), Not(Equals(Select(str(i), Ref(VolumeSize)), "NONE"))
    )
    UseVolumeType = t.add_condition(
        "Vol%s_UseVolumeType" % (i + 1), Not(Equals(Select(str(i), Ref(VolumeType)), "NONE"))
    )
    UseEBSEncryption = t.add_condition(
        "Vol%s_UseEBSEncryption" % (i + 1), Equals(Select(str(i), Ref(EBSEncryption)), "true")
    )
    UseEBSKMSKey = t.add_condition(
        "Vol%s_UseEBSKMSKey" % (i + 1),
        And(Condition(UseEBSEncryption), Not(Equals(Select(str(i), Ref(EBSKMSKeyId)), "NONE"))),
    )
    UseEBSSnapshot = t.add_condition(
        "Vol%s_UseEBSSnapshot" % (i + 1), Not(Equals(Select(str(i), Ref(EBSSnapshotId)), "NONE"))
    )
    UseExistingEBSVolume[i] = t.add_condition(
        "Vol%s_UseExistingEBSVolume" % (i + 1), Not(Equals(Select(str(i), Ref(EBSVolumeId)), "NONE"))
    )
    v[i] = t.add_resource(
        ec2.Volume(
            "Volume%s" % (i + 1),
...
t.add_output(Output("Volumeids", Description="Volume IDs of the resulted EBS volumes", Value=outputs[numberOfVol - 1]))
