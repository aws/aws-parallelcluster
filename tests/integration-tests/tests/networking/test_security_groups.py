...
def test_login_node_security_groups(
    ...
    Test that the network load balancer is referenced in an inbound rule of login node managed security group.
    """
    ...
    cluster = clusters_factory(cluster_config)
    pool_name = cluster_config.login_nodes.pools[0].name
    ec2_client = boto3.client("ec2", region_name=region)
    ...
    if assign_additional_security_groups:
        ...
        logging.info(
            "Asserting that load balancer managed security group has been referenced in an inbound rule "
            "for login node security group"
        )
