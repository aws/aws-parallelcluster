...
def test_custom_munge_key(
    ...
    if use_login_node:
        ...
        _test_munge_key_shared(remote_command_executor_login)
    # Stop compute fleets
    cluster.stop()
    ...
    if use_login_node:
        ...
        cluster.update(update_cluster_stop_login_config)
    ...
    cluster.update(update_cluster_remove_custom_munge_key_config)
...
def _test_custom_munge_key_fetch_and_decode(remote_command_executor, use_custom_munge_key=True):
    ...
    encode_munge_key = result.stdout.strip()
...
def _test_munge_key_shared(remote_command_executor):
    ...
    head_node_munge_key_path = "/opt/parallelcluster/shared_login_node/.munge/.munge.key"
