...
def test_openfoam(
    ...
    for node in number_of_nodes:
        ...
        logging.info(f"Nodes: {node}, Baseline: {baseline_value} seconds, Observed: {elapsed_time} seconds, "
    ...
    if performance_degradation:
        pytest.fail(f"Performance test results show performance degradation for the following nodes:"
                    f"{degraded_nodes}")
    else:
