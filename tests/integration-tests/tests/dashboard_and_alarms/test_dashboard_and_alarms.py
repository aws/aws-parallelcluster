...
def _test_dashboard(cw_client, cluster_name, region, dashboard_enabled, cw_log_enabled):
    ...
    if dashboard_enabled:
        ...
        if cw_log_enabled:
            ...
            assert_that(dashboard_response["DashboardBody"]).contains("Metrics for Common Errors")
        else:
            assert_that(dashboard_response["DashboardBody"]).does_not_contain("Head Node Logs")
            assert_that(dashboard_response["DashboardBody"]).does_not_contain("Metrics for Common Errors")
    else:
        try:
